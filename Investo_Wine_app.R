...
BASE_PRICES <- c(Aperol_Spritz = 18, Prosecco = 18, Vin_Alb = 18, Vin_Rosu = 18)
BOTTLE_COST <- c(Aperol_Spritz = 25.00, Prosecco = 26.00, Vin_Alb = 27.00, Vin_Rosu = 20.00)
EPS_SHARE <- 1e-6
CRISIS_LABEL <- "CRISIS"

JSON_OUTPUT_DIR <- "./JSON"
MASTER_JSON_FILE <- file.path(JSON_OUTPUT_DIR, "wine_live_prices.json")
...
ui <- fluidPage(
  ...
  h1("Investo Society — Wine Market Live Monitoring", style = "text-align:center; color:#00ff88;"),
  ...
  h3("Per-interval details (Price & Quantity by Wine)"),
...
server <- function(input, output, session) {
  ...
  cum_sold <- reactiveVal(setNames(rep(0, 4), names(BASE_PRICES)))
  ...
  write_master_json <- function(index, interval, event_label, price_vec, profit_tick = NA, cumP = NA, is_T0 = FALSE) {
    ...
    if (is_T0) {
      row_obj <- list(
        Interval = interval,
        Aperol_Spritz = as.numeric(price_vec[["Aperol_Spritz"]]),
        Prosecco = as.numeric(price_vec[["Prosecco"]]),
        Vin_Alb = as.numeric(price_vec[["Vin_Alb"]]),
        Vin_Rosu = as.numeric(price_vec[["Vin_Rosu"]])
      )
    } else {
      row_obj <- list(
      ...
      price_history(
        safe_rbind(
          ...
          data.frame(
            ...
            Aperol_Spritz = new_prices["Aperol_Spritz"],
            Prosecco = new_prices["Prosecco"],
            Vin_Alb = new_prices["Vin_Alb"],
            Vin_Rosu = new_prices["Vin_Rosu"],
            Profit = 0,
  ...
  observe({
    ...
    df <- tryCatch(dbGetQuery(pool, "SELECT Aperol_Spritz, Prosecco, Vin_Alb, Vin_Rosu FROM vin"), error = function(e) NULL)
    if (is.null(df)) {
      backfill_done(TRUE)
      return()
    }
  ...
  observe({
    ...
    if (row_index > nrow(df)) {
      if (crisis_triggered()) {
        v_vec <- setNames(rep(0, 4), names(BASE_PRICES))
