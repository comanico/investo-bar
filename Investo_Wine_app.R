# ==========================================================
# INVESTO SOCIETY — Wine MARKET SIMULATION (with Random Crisis)
# ==========================================================
# A clean, single-sector Wine market simulator.
...
CRISIS_WINDOW     <- c(8L, 12L)        # Window in which crisis may occur
CRISIS_MULTIPLIER <- 5.0               # Crisis when any Wine > 2× base
CRISIS_LABEL      <- "CRISIS"
SHOW_CRISIS_LINE  <- TRUE

# --- Market Dynamics ---
ALPHA_SHARE        <- 0.35  # Sensitivity to relative demand
ABS_IMPACT_WEIGHT  <- 0.35  # Sensitivity to absolute demand
LAMBDA             <- 0.80  # EMA smoothing for volume
LAMBDA_SHARE       <- 0.80  # EMA smoothing for shares
DEPTH              <- 8     # Smooth low-demand impact
...
ui <- fluidPage(
  ...
  div(id="mainTitle", "Investo Society — Wine Market"),
  div(id="subTitle", "🍺📈 Live Wine Price Simulation Based on Demand"),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    query <- "
      SELECT id, Aperol, Prosecco, Vin_Alb, Vin_Rosu 
      FROM vin 
      ORDER BY id DESC 
      LIMIT 1
    "
