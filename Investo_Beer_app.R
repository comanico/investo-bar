...
BASE_PRICES <- c(Heineken = 13, Peroni = 13, Corona = 13)
BOTTLE_COST <- c(Heineken = 5.30, Peroni = 5.00, Corona = 6.00)
EPS_SHARE <- 1e-6
CRISIS_LABEL <- "CRISIS"

JSON_OUTPUT_DIR <- "./JSON"
MASTER_JSON_FILE <- file.path(JSON_OUTPUT_DIR, "beer_live_prices.json")
...
ui <- fluidPage(
  ...
  h1("Investo Society — Beer Market Live Monitoring", style = "text-align:center; color:#00ff88;"),
  ...
  h3("Per-interval details (Price & Quantity by Beer)"),
...
server <- function(input, output, session) {
  ...
  write_master_json <- function(index, interval, event_label, price_vec, profit_tick = NA, cumP = NA, is_T0 = FALSE) {
    ...
    if (is_T0) {
      row_obj <- list(
        Interval = interval,
        Heineken = as.numeric(price_vec[["Heineken"]]),
        Peroni   = as.numeric(price_vec[["Peroni"]]),
        Corona   = as.numeric(price_vec[["Corona"]])
      )
    } else {
      row_obj <- list(
      ...
      price_history(
        safe_rbind(
          ...
          data.frame(
            ...
            Heineken = new_prices["Heineken"],
            Peroni = new_prices["Peroni"],
            Corona = new_prices["Corona"],
            Profit = 0,
  ...
  observe({
    ...
    df <- tryCatch(dbGetQuery(pool, "SELECT Heineken, Peroni, Corona FROM bere"), error = function(e) NULL)
    if (is.null(df)) {
      backfill_done(TRUE)
      return()
    }
