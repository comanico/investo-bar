# ==========================================================
# INVESTO SOCIETY — BEER MARKET SIMULATION
...
CRISIS_MODE <- "time_or_spike" # Can be "time_only", "spike_only", or "time_or_spike"
CRISIS_WINDOW <- c(12L, 14L) # Random crisis will be triggered in this interval range
CRISIS_MULTIPLIER <- 2.0 # Crisis triggers if any price > 2x base price
CRISIS_LABEL <- "CRISIS" # Label to mark the crisis event
SHOW_CRISIS_LINE <- TRUE # Visually show crisis moment on the plot


# --- Dynamic Price Sensitivity Parameters ---
ALPHA_SHARE <- 0.40 # Weight of share deviation in pricing
ABS_IMPACT_WEIGHT <- 0.40 # Weight of volume deviation (adjusted dynamically)
LAMBDA <- 0.30 # Smoothing for EMA of volumes
LAMBDA_SHARE <- 0.30 # Smoothing for EMA of share proportions
DEPTH <- 30 # Depth parameter (scales with volume)
...
# --- Warm-up Settings ---
WARMUP_PERIOD <- 5 # Initial intervals where price updates are softened
LAMBDA_RAMP <- 10 # Number of intervals until LAMBDA reaches full value
...
ui <- fluidPage(
  tags$head(tags$style(HTML("
    ...
    table {color:#eee;}
  "))),
  div(id = "mainTitle", "Investo Society — Beer Market (Dynamic Volume Sensitivity)"),
