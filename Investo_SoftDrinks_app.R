# ==========================================================
# INVESTO SOCIETY — Soft Drinks MARKET SIMULATION (with Random Crisis)
# ==========================================================
# A clean, single-sector Soft Drinks market simulator.
...
CRISIS_WINDOW     <- c(4L, 8L)       # Window in which crisis may occur
CRISIS_MULTIPLIER <- 2.0              # Crisis when any Soft Drinks > 2× base
CRISIS_LABEL      <- "CRISIS"
SHOW_CRISIS_LINE  <- TRUE

# --- Market Dynamics ---
ALPHA_SHARE        <- 0.2   # sensitivity to relative demand (smaller than alcohol sectors)
ABS_IMPACT_WEIGHT  <- 0.2   # sensitivity to absolute demand
LAMBDA             <- 0.9   # EMA smoothing for volumes
LAMBDA_SHARE       <- 0.9   # EMA smoothing for shares
DEPTH              <- 12     # deeper smoothing — less volatility
MAX_CHANGE         <- 0.3   # cap per interval (soft market)
ROUND_TO           <- 0.5
SOFT_FLOOR_DELTA   <- 1.0
PROFIT_OFFSET_PER_UNIT <- 1.5
...
ui <- fluidPage(
  ...
  div(id="mainTitle", "Investo Society — Soft Drinks Market"),
  div(id="subTitle", "🍺📈 Live Soft Drinks Price Simulation Based on Demand"),
...
server <- function(input, output, session) {
  ...
  price_history <- reactiveVal(
    data.frame(Index = 0, Interval = "T0", Event = "T0",
  ...
  observe({
    ...
    if (is.null(sheet_cache())) {
      ...
      if (nrow(df) == 0) { cat("⚠️ No numeric rows found.\n"); return() }
    ...
    df <- sheet_cache(); if (is.null(df)) return()
    total <- total_rows(); if (total == 0) return()
    ...
    for (row_index in seq(start, total)) {
      ...
      if (sum(v_vec) == 0) { last_row(row_index); next }
      ...
      ema_volumes(ema_new); ema_shares(sh_new)
      cum_sold(cum_new); cum_profit(cum_profit_new)
      last_row(row_index)
  ...
  output$styledPriceTable <- renderUI({
    hist <- price_history(); if (nrow(hist) == 0) return(NULL)
    latest <- tail(hist, 1)
    tbl <- data.frame(
    ...
    crisis_badge <- if (nzchar(latest$Event[1])) 
      tags$span(style="color:red; font-weight:bold;", paste0("🚨 ", latest$Event[1])) else NULL
